    ...
    if (cartBotton) {
        ...
        cartContents.forEach((item) => {
            if (item.name === name.textContent.trim()) {
                console.log("重複商品");
                item.amount += parseInt(amount.textContent);
                isRepeat = true;
            }
        });
        // 若不是重複商品，則正常加入
        ...
        const card_amount = document.querySelector(
            ".cart .cart-content-amount"
        );
        card_amount.textContent =
            parseInt(card_amount.textContent.trim()) +
        ...
        //     price.textContent.trim().slice(4).replace(/,/g, ""),
        //     amount.textContent
        // );
    }
});
